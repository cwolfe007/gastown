...
- **Phase 2.3**: Updated autonomous managers (polecat, witness, refinery). Polecat session_manager was already updated. Witness manager: removed `internal/claude` import, added `runtime` import, replaced `claude.EnsureSettingsForRole()` with `runtime.EnsureSettingsForRole()`, added `runtime.RunStartupFallback()`. Refinery manager: replaced `t.IsClaudeRunning()` with `t.IsRuntimeRunning()` using `config.ExpectedPaneCommands()` for agent-agnostic detection, replaced `runtimeConfig` with `agentCfg` from `ResolveRoleAgentConfig()`.
